...
import { CalculatedStats, GearSetTalent, ActiveSetInfo, SelectedSkillState, SkillNameId, CalculatedStatsBase } from '../types';
import { SKILL_DEFINITIONS_DATA } from '../constants';
...
const StatsDisplay: React.FC<StatsDisplayProps> = ({ stats, skill1State, skill2State }) => {
  ...
  const renderSkillModStats = (skillId: SkillNameId | null, skillStats: CalculatedStatsBase) => {
    ...
    const filteredRelevantStats = relevantStats.filter(stat => {
        // Check if the value is non-zero for numbers, or not an empty string for strings
        // The value prop of StatItem can be string | number
        const value = (stat.props as any).value;
        return typeof value === 'number' && value !== 0;
      });

    if (filteredRelevantStats.length === 0) return null; // Don't render section if all relevant stats are zero
